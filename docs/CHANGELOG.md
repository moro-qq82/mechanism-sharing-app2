...
### 追加
- アプリケーション起動用バッチファイルの作成 (2025-06-11)
  - `start_app.bat`: アプリケーションを簡単に起動するためのバッチファイル
    - バックエンドとフロントエンドの依存関係を自動でインストール（初回のみ）
    - データベースマイグレーションを自動実行
    - バックエンドサーバー（FastAPI）とフロントエンドサーバー（React）を同時起動
    - Windows環境に最適化された処理フロー
  - `stop_app.bat`: アプリケーションを停止するためのバッチファイル
    - 実行中のNode.jsプロセス（React開発サーバー）を停止
    - 実行中のPythonプロセス（FastAPIサーバー）を停止
  - README.mdの更新
    - 簡単起動セクションを追加し、バッチファイルの使用方法を説明
    - 手動セットアップセクションを整理し、uvの使用を明記
- メカニズム閲覧回数記録機能の実装（2025-05-10）
  - メカニズム詳細画面を開いた回数を記録する機能を実装
  - 個人ごとの閲覧回数と総閲覧回数を記録・表示
  - バックエンド側の実装
    - MechanismViewモデルの作成
    - 閲覧履歴を記録するAPIエンドポイントの実装
    - 閲覧回数を取得するAPIエンドポイントの実装
  - フロントエンド側の実装
    - メカニズム詳細画面で閲覧履歴を記録する処理の追加
    - メカニズム詳細画面で閲覧回数を表示する機能の追加
    - メカニズム一覧画面で閲覧回数を表示する機能の追加
  - テスト実装
    - サービスレイヤーのユニットテスト実装
    - APIエンドポイントのユニットテスト実装
    - 統合テストの実装（2025-05-10）
      - 匿名ユーザーと認証済みユーザーの閲覧履歴記録テスト
      - 複数回閲覧時の閲覧回数増加テスト
      - 複数メカニズムの閲覧回数一括取得テスト
      - メカニズム詳細・一覧APIレスポンスの閲覧回数検証テスト
- 統合テスト実装（2025-05-02）
...
### 修正
- メカニズム詳細画面の閲覧回数重複記録問題の修正 (2025-06-11)
  - 問題：メカニズム詳細画面を開いたときに閲覧回数が2ずつ増える現象が発生していた
  - 原因：React.StrictModeによりuseEffectが開発環境で2回実行され、閲覧履歴記録APIが重複して呼び出されていた
  - 修正内容：
    - `frontend/src/pages/MechanismDetailPage.tsx`にuseRefを使用した重複防止機能を実装
    - 同一コンポーネントインスタンス内での閲覧履歴記録の重複を防ぐロジックを追加
    - 関連するテストコード（`frontend/src/__tests__/pages/MechanismDetailPage.test.tsx`）を更新
  - 結果：ユーザーが確認したところ、変わらず閲覧回数は２ずつ増加していた
...
- メカニズム詳細画面の閲覧回数カウント修正(2025-05-10)
  ...
  - 修正内容：
    ...
    - テストコードも更新して新しい実装に対応
  - 結果：メカニズム詳細画面を開いたときに閲覧回数が1ずつ正しく増加するようになった

- マイグレーション環境の修正(2025-05-10)
  - 問題：バックエンドを起動すると「リレーション"mechanism_views"は存在しません」というエラーが発生
  - 原因：migrations/env.pyファイルにmechanism_viewモデルがインポートされておらず、マイグレーションが正しく適用されていなかった
  - 修正内容：
    - migrations/env.pyファイルにmechanism_viewモデルをインポートするよう修正
    - alembic upgrade headコマンドを実行してマイグレーションを適用
  - 結果：mechanism_viewsテーブルが正しく作成され、メカニズム閲覧回数記録機能が正常に動作するようになった

- バックエンドの認証テスト修正(2025-05-10)
  - 問題：`backend/tests/test_auth.py`のテストが失敗する
  - 原因：
...
- フロントエンドのテスト修正(2025-05-10)
  - 問題：フロントエンドのテストが失敗する
  - 修正内容：
    - `frontend/src/__tests__/services/mechanismService.test.ts`のAPIエンドポイントを修正（`/mechanisms`から`/api/mechanisms`に変更）
    - `frontend/src/__tests__/pages/MechanismListPage.test.tsx`の「閲覧回数の取得に失敗した場合も一覧が表示されること」というテストを修正
    - `frontend/src/__tests__/pages/MechanismNewPage.test.tsx`の「必須フィールドが空の場合にバリデーションエラーが表示されること」というテストを修正
  - 結果：すべてのフロントエンドテストが正常に実行されるようになった

- メカニズム投稿画面でエラーメッセージを投稿ボタンの下に表示するよう修正(2025-05-10)
  - 問題：投稿ボタンを押したときにスクロールしないとエラーが見えないことがあった
  - 修正内容：
