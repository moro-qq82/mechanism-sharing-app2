...
19. メカニズム投稿できるが、詳細画面から「ファイルを表示」しても画像が表示されない（アップロードしたのはpngファイル）
20. サムネイル画像をpngとして登録したが、メカニズム一覧画面で表示されない
21. メカニズム一覧でタイルをクリックして詳細画面に遷移したい
22. メカニズム詳細画面でファイルを表示したときに、ブラウザで表示できない拡張子の場合は自動でダウンロードさせたい
23. 投稿画面で投稿するボタンを押したときにスクロールしないとエラーが見えない。投稿するボタンの下にエラーメッセージを表示したい（必須項目が記入されていません）
